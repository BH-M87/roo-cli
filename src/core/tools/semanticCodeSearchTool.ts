...
import { indexCodeFiles, searchCodeByQuery, initializeRAG } from "../rag"
import { readGlobalSettings } from "../../config/settings"
import { VectorStoreType } from "../rag/vector-store-factory"
...
export const semanticCodeSearchTool: ToolHandler = async ({ toolUse, cwd }) => {
	...
	try {
		...
		logger.debug(`File pattern: ${filePattern || "**/*.{js,ts,jsx,tsx,py,java,c,cpp,h,hpp,cs,go,rb,php}"}`)
		...
		if (results.length === 0) {
			...
			for (let i = 0; i < results.length; i++) {
				...
				resultText += `${i + 1}. File: ${displayPath} (${relevanceScore}% relevance)\n`
				...
				if (chunk.symbols.length > 0) {
					...
					const displaySymbols =
						chunk.symbols.length > 10
							? chunk.symbols.slice(0, 10).join(", ") + `, ... (${chunk.symbols.length - 10} more)`
							: chunk.symbols.join(", ")
		 ...
		 */
		function findRelevantLines(lines: string[], query: string): { start: number; end: number } {
			const queryWords = query
				.toLowerCase()
				.split(/\s+/)
				.filter((w) => w.length > 3)
...
/**
 * Initialize the RAG system based on user settings
 */
async function initializeRAGSystem(): Promise<void> {
	try {
		...
		if (!settings.ragEnabled) {
			logger.info("RAG is disabled in settings, using default in-memory store")
			await initializeRAG()
			return
		}
		...
		if (!ragSettings) {
			logger.info("No RAG settings found, using default configuration")
			await initializeRAG()
			return
		}

		const vectorStoreConfig = ragSettings.vectorStore

		if (vectorStoreConfig.type === "qdrant") {
			if (!vectorStoreConfig.url || !vectorStoreConfig.collectionName) {
				logger.warn("Qdrant configuration incomplete, falling back to in-memory store")
				await initializeRAG()
				return
			}
			...
			await initializeRAG({
				...
				dimensions: vectorStoreConfig.dimensions || 256,
