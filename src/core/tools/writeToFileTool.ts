 ...
 * @returns 是否被截断
 */
function detectCodeOmission(newContent: string, predictedLineCount?: number): boolean {
	// 检查是否有常见的代码省略注释
...
export const writeToFileTool: ToolHandler = async ({ toolUse, cwd }) => {
	...
	const predictedLineCount = params.line_count ? parseInt(params.line_count, 10) : undefined
