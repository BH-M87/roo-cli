...
async function parseFileDefinitions(filePath: string): Promise<string | null> {
	try {
		...
		logger.error(`Error parsing file ${filePath}: ${error instanceof Error ? error.message : String(error)}`)
		return null
	}
}
 ...
 * @returns 定义模式数组
 */
function getDefinitionPatterns(fileExt: string): { name: string; regex: string }[] | null {
	switch (fileExt) {
		case ".js":
		case ".jsx":
		case ".ts":
		case ".tsx":
			return [
				...
				{ name: "function", regex: "(?:export\\s+)?(?:async\\s+)?function\\s+([A-Za-z0-9_$]+)" },
				{
					name: "arrow function",
					regex: "(?:export\\s+)?const\\s+([A-Za-z0-9_$]+)\\s*=\\s*(?:async\\s+)?\\([^)]*\\)\\s*=>",
				},
				{ name: "interface", regex: "(?:export\\s+)?interface\\s+([A-Za-z0-9_$]+)" },
		...
		case ".java":
			return [
				{ name: "class", regex: "(?:public|private|protected)?\\s+class\\s+([A-Za-z0-9_]+)" },
				{ name: "interface", regex: "(?:public|private|protected)?\\s+interface\\s+([A-Za-z0-9_]+)" },
				{ name: "enum", regex: "(?:public|private|protected)?\\s+enum\\s+([A-Za-z0-9_]+)" },
				{
					name: "method",
					regex: "(?:public|private|protected|static)?\\s+(?:void|[A-Za-z0-9_<>\\[\\]]+)\\s+([A-Za-z0-9_]+)\\s*\\(",
				},
			]
		// 其他文件类型的定义模式...
		default:
 ...
 * @returns 工具执行结果
 */
export const listCodeDefinitionsTool: ToolHandler = async ({ toolUse, cwd }) => {
