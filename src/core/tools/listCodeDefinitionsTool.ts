    ...
    for (const file of files) {
      ...
      switch (fileExt) {
        ...
        case ".java":
          // Java 定义
          definitionRegex =
            /(?:class|interface|enum)\s+([A-Za-z0-9_]+)|(?:public|private|protected|static)?\s+(?:void|[A-Za-z0-9_]+)\s+([A-Za-z0-9_]+)\s*\(/g;
          break;
        case ".c":
        case ".cpp":
        case ".h":
        case ".hpp":
          // C/C++ 定义
          definitionRegex =
            /(?:class|struct|enum)\s+([A-Za-z0-9_]+)|(?:void|[A-Za-z0-9_]+)\s+([A-Za-z0-9_]+)\s*\(/g;
          break;
        case ".cs":
          // C# 定义
          definitionRegex =
