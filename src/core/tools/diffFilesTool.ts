 ...
 * @returns 工具执行结果
 */
export const diffFilesTool: ToolHandler = async ({ toolUse, cwd }) => {
  ...
  try {
    ...
    logger.debug(`Comparing files: ${fullPath1} and ${fullPath2}`);
    ...
    logger.error(
      `Error comparing files ${file1} and ${file2}: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
    return `Error comparing files ${file1} and ${file2}: ${
      error instanceof Error ? error.message : String(error)
    }`;
  }
};
