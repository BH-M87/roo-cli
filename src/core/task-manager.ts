...
export class TaskManager {
   ...
   * @returns 任务ID
   */
  createTask(mode: string, cwd: string, systemPrompt: string): string {
    const taskId = uuidv4();
    const now = Date.now();
   ...
   * @returns 更新后的任务
   */
  addUserMessage(taskId: string, content: string): Task | null {
   ...
   * @returns 更新后的任务
   */
  addAssistantMessage(taskId: string, content: string): Task | null {
    const task = this.getTask(taskId);
   ...
   * @returns 更新后的任务
   */
  addToolMessage(taskId: string, content: string): Task | null {
    const task = this.getTask(taskId);
