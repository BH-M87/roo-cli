...
export class TaskSharingService {
	...
	constructor(config: TaskShareConfig = {}) {
		...
		this.shareDir = path.join(process.env.HOME || "~", ".roo-cli", "shared-tasks")
	}
	...
	): Promise<{ success: boolean; shareUrl?: string; shareId?: string; error?: string }> {
		try {
			...
			logger.error(`Failed to share task: ${error instanceof Error ? error.message : String(error)}`)
			return {
				success: false,
	 ...
	 */
	private async uploadToCloud(task: ShareableTask): Promise<{ success: boolean; shareUrl?: string; error?: string }> {
		try {
			if (!this.config.apiEndpoint || !this.config.authToken) {
				throw new Error("Cloud configuration not available")
			}

			const response = await fetch(`${this.config.apiEndpoint}/api/tasks/share`, {
				method: "POST",
				headers: {
			...
			logger.debug(`Cloud upload failed: ${error instanceof Error ? error.message : String(error)}`)
			return {
				success: false,
				error: error instanceof Error ? error.message : String(error),
			}
	...
	private generateDefaultTaskName(taskData: any): string {
		...
		if (taskData.messages && taskData.messages.length > 0) {
			const firstUserMessage = taskData.messages.find((msg: any) => msg.role === "user")
