...
class CodeEmbeddingStore {
	...
	async addEmbedding(embedding: CodeEmbedding): Promise<void> {
		if (this.useVectorStore && this.vectorStore) {
			...
			const metadata = {
				id: this.generateEmbeddingId(embedding.chunk),
				filePath: embedding.chunk.filePath,
				content: embedding.chunk.content,
				startLine: embedding.chunk.startLine,
	...
	/**
	 * Get all code embeddings (only works with in-memory store)
	...
	/**
	 * Find similar code chunks using in-memory search
	 * @param queryEmbedding Query embedding
	 * @param queryText Original query text (for keyword matching)
	 * @param topK Number of results to return
	 * @returns Similar code chunks
...
/**
 * Initialize RAG with Qdrant vector store
 * @param url Qdrant server URL
 * @param collectionName Collection name
 * @param dimensions Vector dimensions
 * @param apiKey Optional API key
 */
export async function initializeRAGWithQdrant(
	url: string,
	collectionName: string,
	dimensions: number = 256,
	apiKey?: string,
): Promise<void> {
	const config: VectorStoreConfigWithType = {
