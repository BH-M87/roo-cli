...
export class InMemoryVectorStore implements VectorStore {
	...
	async addVectors(
		...
		logger.debug(
			`Added ${vectors.length} vectors to in-memory store (total: ${this.vectors.length})`,
		);
		return ids;
	}
	...
	async deleteByIds(ids: string[]): Promise<void> {
		...
		logger.debug(
			`Deleted ${deletedCount} vectors from in-memory store (remaining: ${this.vectors.length})`,
		);
	}
