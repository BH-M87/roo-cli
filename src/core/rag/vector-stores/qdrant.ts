...
export class QdrantVectorStore implements VectorStore {
	...
	async initialize(): Promise<void> {
		try {
			...
			const collectionExists = collections.collections.some(
				(collection: QdrantCollection) => collection.name === this.collectionName,
			)
			...
			throw new Error(`qdrantConnectionFailed:${this.config.url}:${errorMessage}`)
		}
	}
	...
	async addVectors(vectors: number[][], metadata: Record<string, any>[]): Promise<string[]> {
		...
		try {
			...
			logger.debug(`Added ${points.length} vectors to Qdrant collection: ${this.collectionName}`)
	...
	async deleteByIds(ids: string[]): Promise<void> {
		try {
			...
			logger.debug(`Deleted ${ids.length} vectors from Qdrant collection: ${this.collectionName}`)
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : String(error)
	...
	async deleteAll(): Promise<void> {
		try {
			...
			logger.info(`Cleared all vectors from Qdrant collection: ${this.collectionName}`)
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : String(error)
