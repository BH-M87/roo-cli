	...
	ragCommand
		...
		.action(async () => {
			try {
				...
				console.log(
					`  Dimensions: ${settings.vectorStore.dimensions || 'default'}`,
				);

				if (settings.vectorStore.type === 'qdrant') {
					console.log(`  URL: ${settings.vectorStore.url || 'not set'}`);
					console.log(
						`  Collection: ${settings.vectorStore.collectionName || 'not set'}`,
					);
					console.log(
						`  API Key: ${settings.vectorStore.apiKey ? '***set***' : 'not set'}`,
					);
				}
				...
				console.log(
					`  Supported File Types: ${settings.supportedFileTypes.join(', ')}`,
				);
	...
	ragCommand
		...
		.action(async options => {
			try {
				...
				console.log(
					chalk.green('✅ Qdrant configuration updated successfully'),
				);
	...
	ragCommand
		...
		.action(async options => {
			try {
				...
				console.log(
					chalk.green('✅ In-memory configuration updated successfully'),
				);
				console.log(chalk.blue(`   Dimensions: ${options.dimensions}`));
			} catch (error) {
		...
		.action(async options => {
			try {
				...
				if (options.file) {
					...
					console.log(
						chalk.green(`✅ Configuration exported to ${options.file}`),
					);
				} else {
	...
	ragCommand
		...
		.action(async options => {
			try {
				...
				console.log(
					chalk.green(`✅ Configuration imported from ${options.file}`),
				);
			} catch (error) {
