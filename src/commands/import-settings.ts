...
import { safeParseYaml, validateYaml, formatYamlError } from "../utils/yaml-handler";
import { getGlobalRooDirectory, getProjectRooDirectoryForCwd } from "../utils/roo-config";
...
export function createImportSettingsCommand(): Command {
  ...
  command
    ...
    .option("--scope <scope>", "Import scope (global, project, both)", "project")
    ...
    .action(async (filePath: string, options) => {
      try {
        ...
        if (fileExt === ".yaml" || fileExt === ".yml") {
          const parseResult = safeParseYaml<ImportableSettings>(fileContent, filePath);
          if (!parseResult.success) {
            logger.error("Failed to parse YAML settings file:");
            console.error(formatYamlError(parseResult.error!, fileContent, filePath));
            if (!options.force) {
              process.exit(1);
            }
            settings = {};
          ...
          try {
            ...
            logger.error(`Failed to parse JSON settings file: ${error instanceof Error ? error.message : String(error)}`);
            process.exit(1);
          }
        } else {
          logger.error(`Unsupported file format: ${fileExt}. Use .json, .yaml, or .yml`);
          process.exit(1);
        }
...
function validateSettings(settings: ImportableSettings): {
  ...
  if (settings.customInstructions !== undefined && typeof settings.customInstructions !== "string") {
  ...
  const knownProperties = ["rules", "modes", "customInstructions", "allowedCommands", "globalSettings", "projectSettings"];
 ...
 */
async function importGlobalSettings(settings: ImportableSettings, merge: boolean): Promise<void> {
  ...
  if (settings.rules && settings.rules.length > 0) {
    ...
    if (merge) {
      try {
        ...
        await fs.writeFile(rulesFile, `${existingContent}\n\n# Imported Rules\n\n${rulesContent}`);
      } catch {
  ...
  if (settings.globalSettings) {
    ...
    if (merge) {
      try {
        ...
        const mergedSettings = { ...existingSettings, ...settings.globalSettings };
  ...
  if (settings.allowedCommands) {
    ...
    if (merge) {
      try {
        ...
        const mergedCommands = [...new Set([...existingCommands, ...settings.allowedCommands])];
 ...
 */
async function importProjectSettings(settings: ImportableSettings, merge: boolean, cwd: string): Promise<void> {
  ...
  if (settings.projectSettings) {
    ...
    if (merge) {
      try {
        const existingContent = await fs.readFile(settingsFile, "utf-8");
        const existingSettings = JSON.parse(existingContent);
        const mergedSettings = { ...existingSettings, ...settings.projectSettings };
        await safeWriteJson(settingsFile, mergedSettings);
      } catch {
