...
- **1-click task sharing**: Share tasks with others via `roo share` command with support for public, organization, and private visibility levels
- **Global .roo directory rules loading**: Support for loading rules from both global (`~/.roo/rules/`) and project-local (`.roo/rules/`) directories with project overriding global
- **Modes selector improvements**: Enhanced mode selection with new modes (code, ask, debug, orchestrator, auto) and detailed usage guidelines
- **safeWriteJson utility**: Atomic JSON file writing with proper error handling and temporary file usage to prevent data corruption
- **YAML error handling**: Comprehensive YAML parsing with detailed error messages, suggestions, and context information
- **importSettings command**: Import configuration settings from JSON or YAML files with validation and merge capabilities
- **Default task names**: Automatic generation of meaningful task names based on content
- **write_to_file improvements**: Enhanced handling of newline-only content and empty files with better validation
- **Enhanced rules loading**: Support for mode-specific rules and improved rule precedence (global → project-local)

### Improved

- **Task management**: All task operations now use atomic file writing for better reliability
- **Error handling**: Better error messages and validation throughout the application
- **Code organization**: Improved modular structure with dedicated utilities for configuration management
